...
export function Breadcrumbs({ errorPage }) {
  ...
  return (
    <div className="breadcrumbs text-[15px]">
      ...
      {!errorPage && pathnames.map((part, index) => {
        ...
        return (
          <React.Fragment key={to}>
            <span className={isLast ? 'text-black' : 'text-black/50'}>
              {index > 0 && <span> / </span>}
              <Link to={to} className={isLast ? 'font-semibold' : 'hover:text-black'}>
                {part}
              </Link>
            </span>
          </React.Fragment>
